...
export default function SignupScreen() {
  ...
  const submit = async () => {
    ...
    const r = await signup({ name, emailOrPhone, password });
    setBusy(false);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.ivory, padding: spacing.lg, gap: spacing.md }}>
      <Text style={{ fontSize: fontSizes.display, fontWeight: "900", color: colors.ink }}>Join VIDDA.</Text>
      <Field label="Name" value={name} onChange={setName} />
      <Field label="Email or phone" value={emailOrPhone} onChange={setEmailOrPhone} />
      <Field label="Password (8+ characters)" value={password} onChange={setPassword} secure />
      ...
      <Pressable disabled={busy} onPress={submit} style={{ backgroundColor: colors.burgundy, paddingVertical: 16, borderRadius: 999, opacity: busy ? 0.6 : 1 }}>
        <Text style={{ color: colors.white, textAlign: "center", fontWeight: "900", letterSpacing: 1.5 }}>{busy ? "CREATING…" : "CREATE ACCOUNT"}</Text>
      </Pressable>
      <Link href="/auth/login" asChild>
        <Pressable><Text style={{ textAlign: "center", color: colors.ink, fontWeight: "700" }}>Already have an account? Sign in</Text></Pressable>
      </Link>
    </View>
  );
}
